  ...
  useEffect(() => {
    const fetchAndSetPartners = async () => {
      if (partnerSearch.length < 2) {
        // Se o termo de busca for menor que 2 caracteres, usa os parceiros do cache (se existirem)
        const cachedParceiros = sessionStorage.getItem('cached_parceiros');
        if (cachedParceiros) {
          try {
            const parsedCache = JSON.parse(cachedParceiros);
            ...
            const allParceirosData = parsedCache.parceiros || parsedCache;
            if (Array.isArray(allParceirosData)) {
              setAllParceiros(allParceirosData)
              setParceiros(allParceirosData)
              console.log('✅ Parceiros carregados do cache para busca parcial (TitulosReceberTable):', allParceirosData.length);
            } else {
              setAllParceiros([])
              setParceiros([])
            }
          } catch (e) {
            console.error('Erro ao parsear cache para busca parcial:', e);
            setAllParceiros([])
            setParceiros([])
          }
        } else {
          setAllParceiros([])
          setParceiros([]) // Limpa se não houver cache
        }
        return;
      }
                  ...
                  <SelectContent>
                    ...
                    {isLoadingPartners ? (
                      ...
                      <SelectItem value="empty" disabled>
                        {partnerSearch.length < 2 ? "Digite pelo menos 2 caracteres" : "Nenhum parceiro encontrado"}
                      </SelectItem>
                    ) : (
